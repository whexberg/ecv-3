...
export function EventComponent({ event }: { event: CalendarEvent | undefined }) {
    ...
    useEffect(() => {
        if (!event || !event.content) return setContent('');
        parseMDX(event.content, {
            li: (props) => (
    ...
    return (
        <div className="mx-auto max-w-4xl p-6">
            {/* Header */}
            <div className="mb-8">
                <h1 className="mb-4 text-center text-5xl font-bold">{event.title}</h1>

                {event.description && <p className="text-on-page my-4 text-center text-xl">{event.description}</p>}
